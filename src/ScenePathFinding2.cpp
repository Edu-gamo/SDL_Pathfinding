...
void ScenePathFinding2::update(float dtime, SDL_Event *event)
{
	...
	case SDL_MOUSEBUTTONDOWN:
		if (event->button.button == SDL_BUTTON_LEFT)
		{
			...
			{
				if (pathPoints.points.size() > 0)
					if (pathPoints.points[pathPoints.points.size() - 1] == cell2pix(cell))
						break;

				pathPoints.points.push_back(cell2pix(cell));
			}
		}
		break;
	default:
	...
	{
		float dist = Vector2D::Distance(agents[0]->getPosition(), pathPoints.points[currentTargetIndex]);
		if (dist < pathPoints.ARRIVAL_DISTANCE)
		{
			if (currentTargetIndex == pathPoints.points.size() - 1)
			{
				if (dist < 3)
				{
				...
				{
					Vector2D steering_force = agents[0]->Behavior()->Arrive(agents[0], currentTarget, pathPoints.ARRIVAL_DISTANCE, dtime);
		...
		currentTarget = pathPoints.points[currentTargetIndex];
...
void ScenePathFinding2::draw()
{
	...
	for (int i = 0; i < (int)pathPoints.points.size(); i++)
	{
		draw_circle(TheApp::Instance()->getRenderer(), (int)(pathPoints.points[i].x), (int)(pathPoints.points[i].y), 15, 255, 255, 0, 255);
		if (i > 0)
			SDL_RenderDrawLine(TheApp::Instance()->getRenderer(), (int)(pathPoints.points[i - 1].x), (int)(pathPoints.points[i - 1].y), (int)(pathPoints.points[i].x), (int)(pathPoints.points[i].y));
	}
